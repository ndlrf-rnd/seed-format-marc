...
<xsl:stylesheet version="2.0" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="marc">
	...
	<xsl:template name="mrcb">
		...
		<mrcb format-type="bd">
			...
			<xsl:if test="datafield[10&lt;=@tag and @tag&lt;100]">
				<mrcb-numbers-and-codes>
					<xsl:apply-templates select="datafield[10&lt;=@tag and @tag&lt;100]"/>
				</mrcb-numbers-and-codes>
			</xsl:if>
			<xsl:if test="datafield[100&lt;=@tag and @tag&lt;200]">
				<mrcb-main-entry>
					<xsl:apply-templates select="datafield[100&lt;=@tag and @tag&lt;200]"/>
				</mrcb-main-entry>
			</xsl:if>
			<xsl:if test="datafield[200&lt;=@tag and @tag&lt;250]">
				<mrcb-title-and-title-related>
					<xsl:apply-templates select="datafield[200&lt;=@tag and @tag&lt;250]"/>
				</mrcb-title-and-title-related>
			</xsl:if>
			<xsl:if test="datafield[250&lt;=@tag and @tag&lt;300]">
				<mrcb-edition-imprint-etc>
					<xsl:apply-templates select="datafield[250&lt;=@tag and @tag&lt;300]"/>
				</mrcb-edition-imprint-etc>
			</xsl:if>
			<xsl:if test="datafield[300&lt;=@tag and @tag&lt;400]">
				<mrcb-physical-description>
					<xsl:apply-templates select="datafield[300&lt;=@tag and @tag&lt;400]"/>
				</mrcb-physical-description>
			</xsl:if>
			<xsl:if test="datafield[400&lt;=@tag and @tag&lt;500]">
				<mrcb-series-statement>
					<xsl:apply-templates select="datafield[400&lt;=@tag and @tag&lt;500]"/>
				</mrcb-series-statement>
			</xsl:if>
			<xsl:if test="datafield[500&lt;=@tag and @tag&lt;600]">
				<mrcb-notes>
					<xsl:apply-templates select="datafield[500&lt;=@tag and @tag&lt;600]"/>
				</mrcb-notes>
			</xsl:if>
			<xsl:if test="datafield[600&lt;=@tag and @tag&lt;700]">
				<mrcb-subject-access>
					<xsl:apply-templates select="datafield[600&lt;=@tag and @tag&lt;700]"/>
				</mrcb-subject-access>
			</xsl:if>
			<xsl:if test="datafield[700&lt;=@tag and @tag&lt;760]">
				<mrcb-added-entry>
					<xsl:apply-templates select="datafield[700&lt;=@tag and @tag&lt;760]"/>
				</mrcb-added-entry>
			</xsl:if>
			<xsl:if test="datafield[760&lt;=@tag and @tag&lt;800]">
				<mrcb-linking-entry>
					<xsl:apply-templates select="datafield[760&lt;=@tag and @tag&lt;800]"/>
				</mrcb-linking-entry>
			</xsl:if>
			<xsl:if test="datafield[800&lt;=@tag and @tag&lt;=840]">
				<mrcb-series-added-entry>
					<xsl:apply-templates select="datafield[800&lt;=@tag and @tag&lt;=840]"/>
				</mrcb-series-added-entry>
			</xsl:if>
			<xsl:if test="datafield[841&lt;=@tag and @tag&lt;845]">
				<mrcb-holdings-notes>
					<xsl:apply-templates select="datafield[841&lt;=@tag and @tag&lt;845]"/>
				</mrcb-holdings-notes>
			</xsl:if>
			<xsl:if test="datafield[850&lt;=@tag and @tag&lt;=852]">
				<mrcb-location>
					<xsl:apply-templates select="datafield[850&lt;=@tag and @tag&lt;=852]"/>
				</mrcb-location>
			</xsl:if>
			<xsl:if test="datafield[853&lt;=@tag and @tag&lt;=855]">
				<mrcb-captions-and-patterns>
					<xsl:apply-templates select="datafield[853&lt;=@tag and @tag&lt;=855]"/>
				</mrcb-captions-and-patterns>
			</xsl:if>
			...
			<xsl:if test="datafield[863&lt;=@tag and @tag&lt;=865]">
				<mrcb-enumeration-and-chron>
					<xsl:apply-templates select="datafield[863&lt;=@tag and @tag&lt;=865]"/>
				</mrcb-enumeration-and-chron>
			</xsl:if>
			<xsl:if test="datafield[866&lt;=@tag and @tag&lt;=868]">
				<mrcb-textual-holdings>
					<xsl:apply-templates select="datafield[866&lt;=@tag and @tag&lt;=868]"/>
				</mrcb-textual-holdings>
			</xsl:if>
			<xsl:if test="datafield[870&lt;=@tag and @tag&lt;=873]">
				<mrcb-variant-names>
					<xsl:apply-templates select="datafield[870&lt;=@tag and @tag&lt;=873]"/>
				</mrcb-variant-names>
			</xsl:if>
			<xsl:if test="datafield[876&lt;=@tag and @tag&lt;=878]">
				<mrcb-item-information>
					<xsl:apply-templates select="datafield[876&lt;=@tag and @tag&lt;=878]"/>
				</mrcb-item-information>
			</xsl:if>
			<xsl:if test="datafield[880&lt;=@tag and @tag&lt;=886]">
				<mrcb-linkages>
					<xsl:apply-templates select="datafield[880&lt;=@tag and @tag&lt;=886]"/>
				</mrcb-linkages>
			</xsl:if>
		</mrcb>
	</xsl:template>
	...
	<xsl:template match="datafield[@tag='034']/marc:subfield[@code='a']">
		<xsl:element name="mrcb{../@tag}-{@code}">
			<xsl:attribute name="value">
				<xsl:value-of select="text()"/>
			</xsl:attribute>
