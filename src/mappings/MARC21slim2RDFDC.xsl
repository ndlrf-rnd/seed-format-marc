...
<xsl:stylesheet version="1.0" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="marc">
	...
	<xsl:template match="marc:record">
	  ...
	  <rdf:Description>
			...
			<xsl:for-each select="datafield[@tag=260]/marc:subfield[@code='c']">
				<dc:date>
					<xsl:value-of select="."/>
				</dc:date>				
			</xsl:for-each>
		...
		<!--	<xsl:for-each select="datafield[@tag=856]/marc:subfield[@code='q']">
				<dc:format>
					<xsl:value-of select="."/>
				</dc:format>
			</xsl:for-each>-->
...
<xsl:for-each select="datafield[@tag=662]">
			...
			<xsl:for-each select="datafield[@tag=760]|datafield[@tag=762]|datafield[@tag=765]|datafield[@tag=767]|datafield[@tag=770]|datafield[@tag=772]|datafield[@tag=773]|datafield[@tag=774]|datafield[@tag=775]|datafield[@tag=776]|datafield[@tag=777]|datafield[@tag=780]|datafield[@tag=785]|marc:datafield[@tag=786]|marc:datafield[@tag=787]">
				<dc:relation>
					<xsl:call-template name="subfieldSelect">
