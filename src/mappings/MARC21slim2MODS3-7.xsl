<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="xlink marc" version="1.0">
	...
	<xsl:template name="marcRecord">
		...
		<!-- titleInfo -->
		<!-- 1.121 -->
		<xsl:for-each select="datafield[@tag='245'] | datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')]">
			<xsl:call-template name="createTitleInfoFrom245"/>
		</xsl:for-each>
		<!-- 1.121 -->
		<xsl:for-each select="datafield[@tag='210'] | datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'210')]">
			<xsl:call-template name="createTitleInfoFrom210"/>
		</xsl:for-each>
		<!-- 1.121 -->
		<xsl:for-each select="datafield[@tag='246'] | datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'246')]">
			<xsl:call-template name="createTitleInfoFrom246"/>
		</xsl:for-each>
		<!-- 1.121 -->
		<xsl:for-each select="datafield[@tag='240'] | datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'240')]">
			<xsl:call-template name="createTitleInfoFrom240"/>
		</xsl:for-each>
		<!-- 1.121 -->
		<xsl:for-each select="datafield[@tag='740'] | datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'740')]">
			<xsl:call-template name="createTitleInfoFrom740"/>
		</xsl:for-each>
		<!-- 1.121 -->
		<xsl:for-each select="datafield[@tag='130'] | datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')]">
