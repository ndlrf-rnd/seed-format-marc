...
const buildStack = (schema, e, output) => {
  ...
  if (output && (typeof output.write === 'function')) {
    const errMsg = `JSONATA Error: ${JSON.stringify(errObj)}\n`;
...
const jsonataRunner = (schema, output = process.stderr) => {
  ...
  return (runnerInput, runnerOutput = process.stderr, safe = true) => {
    try {
      const evalResult = jsn.evaluate(runnerInput);
      if (safe) {
        return JSON.parse(JSON.stringify(evalResult));
      }
      return evalResult;
    } catch (e) {
      return buildStack(schema, e, runnerOutput);
    }
  };
};
...
const registerJsonata = (filePath) => {
  ...
  return (input, ...args) => global.REGISTRY_JSONATA[filePath](input, ...args);
};
